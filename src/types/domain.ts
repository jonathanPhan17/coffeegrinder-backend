...
/* -- Run quotas (section 8: GET /quota) ------------------------------- */

export interface QuotaWindow {
  used: number;
  limit: number;
  /** max(0, limit - used) -- precomputed so the frontend never derives it. */
  remaining: number;
  /** ISO timestamp of the next UTC month start (monthly) / next UTC midnight (daily). */
  resetsAt: string;
}
